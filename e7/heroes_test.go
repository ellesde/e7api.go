...
func TestHeroesService_GetByID(t *testing.T) {
...
func TestHeroesService_List(t *testing.T) {
	client, mux, _, teardown := setup()
	defer teardown()
	...
	ctx := context.Background()
	got, _, err := client.Heroes.List(ctx)
	if err != nil {
		t.Errorf("Heroes.List returned error: %v", err)
	}
	...
	if diff := cmp.Diff(want, got); diff != "" {
		t.Errorf("Heroes.List mismatch (-want +got):\n%s", diff)
	}

	// Test s.client.NewRequest failure
	client.BaseURL.Path = ""
	got, resp, err := client.Heroes.List(ctx)
	if got != nil {
		t.Errorf("client.BaseURL.Path='' List = %#v, want nil", got)
	}
	if resp != nil {
		t.Errorf("client.BaseURL.Path='' List resp = %#v, want nil", resp)
	}
	if err == nil {
		t.Error("client.BaseURL.Path='' List err = nil, want error")
	}
}
